...
| Clarify | `atlas_expand_context` | Expand exactly one named `code:`, `entity:`, `design:`, `memory:` or task-bound `visual:`, `visual-review:`, `figma-snapshot:`, `delivery:`, `contract:`, `continuation:`, `retrieval:`, `manifest:`, or `receipt-*` handle. Task-bound handles require the exact `task_id`. |
...
| Continue or close | `atlas_task_state` | Recover only an unambiguous exact-checkout task, record the immutable evidence contract, checkpoint criterion progress, persist a bounded Figma snapshot, block, or record the immutable technical outcome. Success is gated by required criteria and is not proof of external delivery. |
