...
| --- | --- | --- |
| Prepare | `atlas_prepare_task` | Resolve the task-scoped source ledger, reuse the focused checkout-and-branch task by default, refresh code evidence, rank reuse, and return a stable task ID plus bounded handles. |
| Clarify | `atlas_expand_context` | Expand exactly one named `code:`, `entity:`, `design:`, `memory:` or task-bound `visual:`, `visual-review:`, `figma-snapshot:`, `delivery:`, `contract:`, `continuation:`, `feedback:`, `git-state:`, `retrieval:`, `manifest:`, or `receipt-*` handle. Infer identity from a task-owned handle and reject an explicit conflicting `task_id`. |
...
| Continue or close | `atlas_task_state` | Recover focused state, append feedback, reconcile Git and sparse criterion progress, record evidence, block, or record the immutable technical outcome. Success is gated by required criteria and feedback and is not proof of external delivery. |
...
focused `task_id` for the same objective. Pass `start_new_task: true` only for a
separate objective.
...
Atlas is a knowledge/evidence sidecar. It does not own native task, ticket,
agent, terminal, branch, or worktree lifecycle.
